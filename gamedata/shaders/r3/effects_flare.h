...
inline float get_depth(float2 uv) { return s_position.Load(int3(uv * screen_res.xy, 0), 0).z; }
...
float4 generate_flare(float2 uv)
{
    ...
    if (!any(dep) && !is_sky(get_depth(get_sun_uv())))
    {
        return float4(0.f, 0.f, 0.f, 0.f);
    }

    float4 _final = float4(col * L_sun_color, 1.0);

    float fadeFactor = saturate(max(max(dep.x, dep.y), max(dep.z, dep.w)));

    float4 final = max(lerp(float4(0.f, 0.f, 0.f, 0.f), _final, fadeFactor), _final);
